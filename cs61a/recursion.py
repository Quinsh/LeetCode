...
# LAB 8 - Mutable Trees, Linked Lists
class Tree:
    """
...
# t = Tree(2, [Tree(1, [Tree(3), Tree(4), Tree(5)]), Tree(6, [Tree(7)])])
# cumulative_mul(t)
# print_tree(t)
...
# t2 = Tree(1, [Tree(2, [Tree(4), Tree(5)]), Tree(3, [Tree(6), Tree(2)]), Tree(2, [Tree(6),  Tree(2), Tree(7), Tree(8)]), Tree(4)])
# delete(t2, 2)
# print_tree(t2)
...
#q5
# # optional challenge
# # it flattens the nested linked list, idk if it is supposed to. If it's not supposed to, just change lst.extend(convert_list(cur.first)) to lst.append(...)
# # linked list traversal method. (iterative way)
# def convert_link(link):
#     lst = []
#     cur = link
#     while (cur is not Link.empty):
#         if type(cur.first) == int:
#             lst.append(cur.first)
#         else:
#             lst.extend(convert_link(cur.first))
#         cur = cur.rest
#     return lst
...
#     else:
#         lst.append[convert_link(link.first)]

#     return lst + convert_link(link.rest)


#q6:
# # optional challenge (actually optional challenge doesn't add anything here. it'd be the same without the challenge)
...
def store_digits(n):
    def helper(n, tens):
        if not tens: return Link.empty
        return Link(n // tens, helper(n % tens, tens // 10))

    tens = 10 ** (int(math.log10(n)))
    return helper(n, tens)
